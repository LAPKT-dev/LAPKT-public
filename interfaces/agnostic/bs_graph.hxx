		...
		void find_conflicts( std::vector<Fluent_Vec>& vec ) {
			for ( int k = m_nodes.size() - 1; k > 0; k-- ) {
				...
				for ( int l = m_nodes.size()-1; l >= 0; l-- ) {
					if ( !m_valid[l] || k == l || m_nodes[l]->rank < m_nodes[k]->rank ) continue;
					Fluent_Vec cond;
					if ( l == 0 )
						cond = m_model.goal();
					else {
						Fluent_Vec tmp_cond = m_nodes[l]->action->pre();
						for ( unsigned i = 0; i < m_nodes[l]->eff_idx.size(); i++ ) {
							if ( m_nodes[l]->eff_idx[i] == 0 ) continue;
							const CC_Problem::CC_Cond_Eff& ce = m_nodes[l]->action->cond_effs()[  m_nodes[l]->eff_idx[i] ];
					...
					if ( !exists_path_between( k, l, kl_path ) ) {
						...
						unsigned min_rank = ( m_nodes[k]->rank > m_nodes[l]->rank ? m_nodes[k]->rank : m_nodes[l]->rank  );
						for ( int i = m_nodes.size()-1; i >= 0; i-- ) {
							if ( !m_valid[i] || i == k || i == l || m_nodes[i]->rank <= min_rank ) continue;
