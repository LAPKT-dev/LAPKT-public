...
def time_taken(task_name: str):
    ...
    print(
        (
            ...
            + "seconds CPU time***"
        ).format(task_name, time() - start[0], process_time() - start[1])
    )
    print("----------------------------------------------------------")
    stdout.flush()
...
def exists_exec(runfile, name):
    if runfile and isfile(runfile) and access(runfile, X_OK):
        ...
        [
            isfile(join(path, name)) and access(join(path, name), X_OK)
...
class Planner:
    ...
    def validate(self):
        ...
        if retval == -1:
            print("Plan was not validated, since 'validate'" + " binary was not found")
        else:
    ...
    def _load_problem(self):
        ...
        if self.config["grounder"]["value"] == "Tarski":
            ...
            raise ValueError(
                "The value doesn't match supported parsers -" + " Tarski/FF/FD"
            )
        ...
        if self.config["grounder"]["value"] == "FF":
            process_task(
                ...
                self.planner_instance.ignore_action_costs,
                False,
            )
        elif self.config["grounder"]["value"] in ["Tarski", "FD"]:
            process_task(
                self.config["domain"]["value"],
                self.config["problem"]["value"],
                self.planner_instance,
    ...
    def _exec(self):
        ...
        self.planner_instance.setup(
            bool(
                not (
                    self.config.get("no_match_tree", None)
                    and self.config["no_match_tree"]["value"]
                )
            )
        )
