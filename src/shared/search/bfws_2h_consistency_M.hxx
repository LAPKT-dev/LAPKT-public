...
#ifndef __SINGLE_QUEUE_DOUBLE_HEURISTIC_GREEDY_BEST_FIRST_SEARCH_FAST_EXP_CONSISTENCY_M__
#define __SINGLE_QUEUE_DOUBLE_HEURISTIC_GREEDY_BEST_FIRST_SEARCH_FAST_EXP_CONSISTENCY_M__
...
class BFWS_2H_Consistency_M :
		public BFWS_2H_Consistency<Search_Model, First_Heuristic, Second_Heuristic, Relevant_Fluents_Heuristic, Open_List_Type >  {
...
public:
	...
	virtual void 			process(  Search_Node *head ) {
		...
		for (unsigned i = 0; i < app_set.size(); ++i ) {
			...
			this->eval( n );
			if( n->h1n() == std::numeric_limits<unsigned>::max() ){ //rel_plan infty
#ifdef DEBUG
				if ( m_verbose ) {
