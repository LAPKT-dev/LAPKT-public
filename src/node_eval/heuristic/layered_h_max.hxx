		...
		protected:
			...
			void update(unsigned p, unsigned v, unsigned act_idx, unsigned eff_idx)
			{
				...
				for (unsigned eff_idx : m_strips_model.relevant_effects(p))
				{
					...
					std::cout << "action = " << (m_strips_model.effects()[eff_idx].act_idx == no_such_index ? "(init)" : m_strips_model.actions()[m_strips_model.effects()[eff_idx].act_idx]->signature());
			...
			void compute(const State &s)
			{
				...
				do
				{
					...
					for (auto eff_index : m_current_effects)
					{
						...
						std::cout << "action = " << (m_strips_model.effects()[eff_index].act_idx == no_such_index ? "(init)" : m_strips_model.actions()[m_strips_model.effects()[eff_index].act_idx]->signature());
