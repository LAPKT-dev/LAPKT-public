...
RPIW_Planner::RPIW_Planner()
		: STRIPS_Interface(), m_iw_bound(2), m_log_filename("rpiw.log"), m_plan_filename("plan.ipc")
{
}
...
void RPIW_Planner::setup(bool gen_match_tree)
{
...
void RPIW_Planner::solve()
{
	...
	if (m_atomic)
		iw_t = do_search_single_goal(engine, search_prob.task(), plan_stream);
	else
		iw_t = do_search(engine, search_prob.task(), plan_stream);
