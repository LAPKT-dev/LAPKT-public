...
AT_BFS_f_Planner::AT_BFS_f_Planner(std::string domain_file, std::string instance_file)
		: STRIPS_Interface(domain_file, instance_file), m_iw_bound(3),
			m_max_novelty(2), m_log_filename("planner.log"), m_plan_filename("plan.ipc"), m_enable_siw(true), m_enable_bfs_f(true)
{
}
...
void AT_BFS_f_Planner::setup(bool gen_match_tree)
{
...
float AT_BFS_f_Planner::do_stage_1(SIW_Fwd &engine, float &cost)
{
...
float AT_BFS_f_Planner::do_stage_3(Anytime_RWA &engine, float B, float &cost)
{
	engine.start(B);
...
float AT_BFS_f_Planner::do_stage_2(Anytime_GBFS_H_Add_Rp_Fwd &engine, float B, float &cost)
{
...
void AT_BFS_f_Planner::report_no_solution(std::string reason)
{
	std::ofstream plan_stream(m_plan_filename.c_str());
