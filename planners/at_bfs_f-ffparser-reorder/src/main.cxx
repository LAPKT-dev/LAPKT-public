...
std::pair<float, std::vector< aptk::Action_Idx > >  do_stage_3(  STRIPS_Problem& prob, Anytime_RWA& engine, float B, float& cost,std::ofstream& details, std::string plan_filename  ) {
	engine.start(B);
    ...
    return std::pair<float, std::vector< aptk::Action_Idx > > (total_time, final_plan);
}

std::pair<float, std::vector< aptk::Action_Idx > > do_stage_2(  STRIPS_Problem& prob, Anytime_GBFS_H_Add_Rp_Fwd& engine, float B, float& cost,std::ofstream& details, std::string plan_filename  ) {
...
void write_reordered_plan(std::string reordered_plan_filename, aptk::STRIPS_Problem & plan_prob, std::map<int, std::list<aptk::Action_Idx>>& p_plan)
{
    std::ofstream myfile;
...
void process_plan(aptk::State & goal, Fwd_Search_Problem & search_prob, aptk::State & root, STRIPS_Problem & prob, std::string & plan_filename, std::pair<float, std::vector< aptk::Action_Idx > > & result)
{

    aptk::Fluent_Vec backup_init = prob.init();
    aptk::Fluent_Vec backup_goal = prob.goal();

    STRIPS_Problem::set_init( prob, root.fluent_vec() );
    STRIPS_Problem::set_goal( prob, goal.fluent_vec() );
    ...
    write_reordered_plan(plan_filename, prob, reordered_plan);

    STRIPS_Problem::set_init( prob, backup_init );
    STRIPS_Problem::set_goal( prob, backup_goal );
}
