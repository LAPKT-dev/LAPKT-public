...
#ifndef __EPISTEMIC_SIW__
#define __EPISTEMIC_SIW__
...
class Epistemic_SIW : public Serialized_Search<Search_Model, 
					       Epistemic_IW<Search_Model, 
							    aptk::agnostic::Novelty_Partition<Search_Model, Epistemic_Node< aptk::State > >,
					       ...
					       Epistemic_Node< aptk::State >> {
    
public:

	typedef  	 Epistemic_Node< aptk::State >                            				Search_Node;
	...
	Epistemic_SIW( const Search_Model& search_problem ) 
	    : Serialized_Search<Search_Model, Epistemic_IW<Search_Model, aptk::agnostic::Novelty_Partition<Search_Model, Search_Node>, RP_Heuristic>, Search_Node>( search_problem ), m_sum_exp_count(0), m_sum_gen_count(0), m_pruned_sum_B_count(0), m_sum_B_count(0), m_max_B_count(0), m_iw_calls(0), m_max_bound( std::numeric_limits<unsigned>::max() ) {	   	
		m_goal_agenda = NULL;
	}

	virtual ~Epistemic_SIW() {
	}
	...
	virtual bool	find_solution( float& cost, std::vector<Action_Idx>& plan ) {
		...
		if( m_goal_agenda ){
		    ...
		    unsigned goal_idx = this->problem().task().goal()[0];
		    aptk::Fluent_Vec new_goals = this->problem().task().actions_adding( goal_idx )[0]->prec_vec();
		    for( auto ng : new_goals )	    
			this->m_goal_candidates.push_back(ng);
			    
//std::cout<< this->problem().task().fluents()[ ng ]->signature()<<" ACHIEVED!\n ";
		}
		...
		do{
			...
			if ( end == NULL ) {
				 ...
				 */
			    // end->state()->print(std::cout);
			    // unsigned goal_idx = this->problem().task().goal()[0];
			    // aptk::Fluent_Vec new_goals = this->problem().task().actions_adding( goal_idx )[0]->prec_vec();
			    // for( auto ng : new_goals )	    
			    // 	if( end->state()->fluent_set().isset(ng))
			    // 	    std::cout<< this->problem().task().fluents()[ ng ]->signature()<<" ACHIEVED!\n ";
				...
				unsigned goal_idx = this->problem().task().goal()[0];
				Action * goal_act = this->problem().task().actions_adding( goal_idx )[0];
				aptk::Fluent_Vec new_goals = goal_act->prec_vec();
				if(end->state()->entails( new_goals )){
				    State *succ = this->problem().next( *(end->state()), goal_act->index() );	       			
				    plan.push_back(goal_act->index());

				    Search_Node* n = new Search_Node( succ , goal_act->index(), end, goal_act->cost(), false );
				    end = n;
				}
