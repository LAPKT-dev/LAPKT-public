...
const		unsigned Action_Idx no_op = -1; 
...
public:
	...
	virtual bool	goal( const State& s ) = 0;
	// MRJ: Returns true if a is applicable on state s
	virtual bool	is_applicable( const State& s, Action_Idx a ) = 0;	
	// MRJ: Returns cost of executing action a on state s
	virtual float	cost( const State& s, Action_Idx a ) = 0;
	// MRJ: Returns the state resulting from applying action a on state s
	virtual void	next( const State& s, Action_Idx a, State* sp ) = 0;
